...
#define MAX_BUFFERED_TUPLES 1000
	...
	/*
	 * The target must be a plain relation, foreign table or have an INSTEAD
	 * OF INSERT row trigger.  (Currently, such triggers are only allowed on
	 * views, so we only hint about them in the view case.)
	 */
	 ...
	 * it if the table is partitioned. If table is foreign, we feed it tuples
	 * one-by-one too.
	 */
